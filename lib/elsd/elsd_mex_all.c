...
static image_double read_mex_image_double(const mxArray *im)
{
    unsigned int xsize,ysize,depth,x,y;
    xsize= mxGetM(im);
    ysize= mxGetN(im);
    double *im_ptr = mxGetPr(im);
    image_double image;
    ...
    for(y=0;y<ysize;y++)
        for(x=0;x<xsize;x++)
            image->data[ x + y * xsize ] = im_ptr[ x + y * xsize ];
...
void EllipseDetection(image_double image,double rho,double prec,double p,
        ...
        int *circ_count,int *line_count,char *fstr,double density_th)
{
    ...
    //printf("db: eps=%f,%f;  density_th=%f; prec=%f\n",eps,mlog10eps,density_th,prec);
    double reg_angle;
    int pext[8];
    ...
    for(;list_p; list_p = list_p->next)
    {
                ...
                angles->data[list_p->y*angles->xsize+list_p->x] != NOTDEF)
        {
            ...
            if (reg_size>2)
            {
                ...
                tmp_used=0;
                if(nfa[2]>mlog10eps && regp_size[2]>min_size[2]) /* ellipse */
                {
                    tmp_used=1;
                    //printf("ellp:%d,%f,%f,%f\n",(*ell_count),nfa[2],mlog10eps,eps);
                    (*ell_count)++;
                    if (smooth){
                        for(i=0;i<4;i++){
                            parame[i] = parame[i]*1.25;
                            pext[i] = pext[i+4]*1.25;
                        }
                    }
                    ang_start = atan2(pext[5]-parame[1],pext[4]-parame[0]);
                    ang_end = atan2(pext[7]-parame[1],pext[6]-parame[0]);
                    
                    double a = pow((parame[2]-parame[3])/(parame[2]+parame[3]),2);
                    double C = M_PI*(parame[2]+parame[3])*(1+3*a/(10+sqrt(4-3*a)));
                    ...
                    fprintf(fe,"%d %f %f %f %f %f %f %f %f\n",2,parame[0]+1,parame[1]+1,parame[2],parame[3],parame[4],ang_start,ang_end,nfa[2]);
                ...
                if(nfa[1]>mlog10eps && regp_size[1]>min_size[1]) /* circle */
                {
                    tmp_used=1;
                    (*circ_count)++;
                    if (smooth){
                        for(i=0;i<4;i++){
                            paramc[i] = paramc[i]*1.25;
                            pext[i] = pext[i]*1.25;
                        }
                    }
                    ang_start = atan2(pext[1]-paramc[1],pext[0]-paramc[0]);
                    ang_end = atan2(pext[3]-paramc[1],pext[2]-paramc[0]);
                    
                    double C = M_2__PI * paramc[2];
                    if (angle_diff(ang_start,ang_end)<M_2__PI*SQRT2/C && angle_diff_signed(ang_start,ang_end)>0){
                        ang_start=0;
                        ang_end=2*M_PI;
                    }else{
                    ...
                    // matlab: indexing from 1
                    fprintf(fe,"%d %f %f %f %f %f %f %f %f\n",1,paramc[0]+1,paramc[1]+1,paramc[2],paramc[3],paramc[4],ang_start,ang_end,nfa[1]);
                 ...
                 if(nfa[0]>mlog10eps && regp_size[0]>min_size[0]) /* line */
                {
                     tmp_used=1;
                    (*line_count)++;
                    if (smooth){
                    ...
                    // for easy matlab 'load'
                    // matlab index from 1
                    fprintf(fe,"%d %f %f %f %f %d %d %d %f \n",0,1+lin[0],1+lin[1],1+lin[2],1+lin[3],0,0,0,nfa[0]);
    ...
    // if (smooth) free_image_double(imgauss);else free_image_double(image);
    // don't free image, as it's now freed by mex
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    double ang_th=tmp[0];
    double density_th=tmp[1];
    double eps =tmp[2];
    int smooth = (int)tmp[3];
    ...
    image = read_mex_image_double(prhs[0]);
    ...
    EllipseDetection(image, rho, prec, p, eps, smooth, &ell_count, &circ_count,
            &line_count,buf,density_th);
    plhs[0] =mxCreateDoubleScalar((double)line_count);
    plhs[1] =mxCreateDoubleScalar((double)circ_count);
    plhs[2] = mxCreateDoubleScalar((double)ell_count);
    mxFree(buf);
    //plhs[0]=mexCreateScalarDouble(line_count);
